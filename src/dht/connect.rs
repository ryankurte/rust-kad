...
use super::{Dht, Operation, OperationKind, OperationState, RequestState};
    ...
    Store: Datastore<Id, Data> + Send + 'static,
{
    ...
    pub fn connect(
        &mut self,
        peers: &[Entry<Id, Info>],
    ) -> Result<(ConnectFuture<Id, Info>, ReqId), Error> {
    ...
    pub fn connect_start(
        &mut self,
    ) -> Result<(ConnectFuture<Id, Info>, ReqId, Request<Id, Data>), Error> {
        let req_id = ReqId::generate();
        let (done_tx, done_rx) = mpsc::channel(1);
...
mod tests {
    ...
    async fn test_connect() {
        ...
        let mut dht: Dht<_, u32, u32, u16> = Dht::standard(*n1.id(), config, tx);

        info!("Start connect");
        // Issue lookup
        let (connect, req_id) = dht
            .connect(&[n2.clone(), n3.clone()])
        ...
        assert_eq!(
            rx.try_next().unwrap(),
            Some((req_id, n3.clone(), Request::FindNode(*n1.id())))
        );
        assert_eq!(
            rx.try_next().unwrap(),
            Some((req_id, n2.clone(), Request::FindNode(*n1.id())))
        );
        ...
        dht.handle_resp(
            req_id,
            &n2,
            &Response::NodesFound(*n1.id(), vec![n4.clone()]),
        )
        .unwrap();
        dht.handle_resp(
            req_id,
            &n3,
            &Response::NodesFound(*n1.id(), vec![n5.clone()]),
        )
        .unwrap();
        ...
        assert_eq!(
            rx.try_next().unwrap(),
            Some((req_id, n4.clone(), Request::FindNode(*n1.id())))
        );
        assert_eq!(
            rx.try_next().unwrap(),
            Some((req_id, n5.clone(), Request::FindNode(*n1.id())))
        );
        ...
        assert_eq!(connect.await, Ok(vec![n4, n5, n3, n2]));
    }
}
