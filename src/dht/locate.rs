...
mod tests {
    ...
    async fn test_lookup() {
        ...
        info!("Start locate");
        // Issue lookup
        let (lookup, req_id) = dht.locate(*n4.id()).expect("Error starting lookup");
        ...
        assert_eq!(
            rx.try_next().unwrap(),
            Some((req_id, n2.clone(), Request::FindNode(*n4.id())))
        );
        assert_eq!(
            rx.try_next().unwrap(),
            Some((req_id, n3.clone(), Request::FindNode(*n4.id())))
        );
        ...
        dht.handle_resp(
            req_id,
            &n3,
            &Response::NodesFound(*n4.id(), vec![n4.clone()]),
        )
        .unwrap();
        dht.handle_resp(
            req_id,
            &n2,
            &Response::NodesFound(*n4.id(), vec![n5.clone()]),
        )
        .unwrap();
        ...
        assert_eq!(
            rx.try_next().unwrap(),
            Some((req_id, n4.clone(), Request::FindNode(*n4.id())))
        );
        assert_eq!(
            rx.try_next().unwrap(),
            Some((req_id, n5.clone(), Request::FindNode(*n4.id())))
        );
